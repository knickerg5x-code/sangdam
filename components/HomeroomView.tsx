    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 animate-in fade-in">
        <div className="bg-white p-8 rounded-3xl border shadow-xl max-w-sm w-full text-center">
          <h3 className="text-xl font-black text-slate-800 mb-6">담임 교사 로그인</h3>
          <input
            type="text"
          ...
          <button
            ...
            className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black disabled:opacity-50 hover:bg-blue-700 transition-all"
          >
            접속하기
          </button>
        </div>
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setIsLoggedIn(false); localStorage.removeItem('last_homeroom_name'); }} className="text-[10px] text-slate-400 hover:text-slate-600 font-bold underline">변경</button>
        </div>
        <button
      ...
      {isFormOpen && (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-3xl border-2 border-blue-50 shadow-xl space-y-6 animate-in slide-in-from-top-4 duration-300">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-slate-200 text-center text-xs">
                ...
                <tbody>
                  {PERIODS.map(p => (
                    <tr key={p}>
                      <td className="border p-2 font-bold bg-slate-50">{p}교시</td>
                      {DAYS.map(d => {
                        ...
                        return (
                          <td 
                            ...
                            className={`border p-3 cursor-pointer transition-colors hover:bg-blue-50 ${isSelected ? 'bg-blue-500 text-white' : 'bg-white text-transparent'}`}
                          >
                            ✓
                          </td>
                        );
          ...
          <div className="space-y-1">
            ...
            <textarea placeholder="상담이 필요한 구체적인 내용을 입력하세요." className="w-full p-4 bg-slate-50 rounded-2xl border-0 h-24 text-sm" value={formData.reason} onChange={e => setFormData({ ...formData, reason: e.target.value })} />
          </div>
      ...
      <div className="grid gap-4">
        {requests.filter(r => r.requesterName === teacherName.trim()).length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-200 text-slate-400">내가 신청한 내역이 없습니다.</div>
        ) : (
          requests.filter(r => r.requesterName === teacherName.trim()).map(req => (
            <div key={req.id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm transition-all hover:shadow-md">
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className="font-black text-lg text-slate-800">{req.studentClass} {req.studentName}</h4>
                    <span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-[10px] font-bold">{req.subject}</span>
                  </div>
                  <p className="text-xs text-slate-400 font-medium italic">담당강사: {req.assignedInstructorName}</p>
                </div>
                
                <div className="text-right">
                  {req.proposedDay ? (
                    <div className="flex flex-col items-end gap-2">
                      <div className="px-3 py-1.5 bg-blue-50 border border-blue-100 text-blue-700 rounded-xl text-xs font-black">
                        강사 제안: {req.proposedDay}요일 {req.proposedTime}
                      </div>
                      {!req.isDeliveryConfirmed && (
                        <button 
                          onClick={() => onUpdateStatus(req.id, { isDeliveryConfirmed: true })}
                          className="px-4 py-2 bg-slate-800 text-white rounded-xl text-xs font-black hover:bg-slate-700 transition-all shadow-sm"
                        >
                          과목 선생님에게 전달 완료
                        </button>
                      )}
                      {req.isDeliveryConfirmed && (
                        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-xl text-xs font-black">
                    ...
                    <span className="text-[10px] text-slate-300 font-bold uppercase tracking-widest animate-pulse">상담 시간 조율 중...</span>
                  )}
                </div>
              </div>
            </div>
