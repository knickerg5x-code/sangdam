    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="bg-white p-8 rounded-3xl border shadow-xl max-w-sm w-full text-center">
          <h3 className="text-xl font-black text-slate-800 mb-6 tracking-tighter">강사 모드 접속</h3>
          <input
            type="text"
  ...
  return (
    <div className="space-y-6">
      <div className="flex bg-slate-100 p-1.5 rounded-[1.5rem] w-full max-w-xs mx-auto shadow-inner">
        <button onClick={() => setActiveTab('PENDING')} className={`flex-1 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'PENDING' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400'}`}>신청 목록</button>
      ...
      <div className="grid gap-6">
        {filteredRequests.map(req => (
          <div key={req.id} className={`p-8 rounded-[2.5rem] border shadow-md transition-all relative overflow-hidden ${req.isDeliveryConfirmed ? 'bg-blue-50/70 border-blue-300' : 'bg-white border-slate-200'}`}>
            {req.isDeliveryConfirmed && (
              <div className="absolute top-0 right-0 px-6 py-2 bg-blue-600 text-white text-[10px] font-black rounded-bl-2xl shadow-lg z-10 flex items-center gap-1.5 animate-in slide-in-from-top-2">
            ...
            <div className="flex flex-col md:flex-row gap-10">
              <div className="shrink-0 space-y-3">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block text-center">담임 제안 시간</span>
              ...
              <div className="flex-1 space-y-6">
                ...
                {activeTab === 'PENDING' ? (
                  <div className="space-y-6 pt-4 border-t border-slate-100">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <select 
                          className="w-full p-4 bg-slate-50 rounded-2xl border-0 text-sm font-black focus:ring-2 focus:ring-blue-500 shadow-sm" 
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">조치 및 상담 내용</label>
                      <textarea 
                        placeholder="학생과 상담한 후의 조치 내용이나 특이사항을 기록하세요." 
                  ...
                  <div className="p-6 bg-emerald-50/50 rounded-[2rem] border border-emerald-100">
                    <p className="text-[10px] font-black text-emerald-600 mb-3 tracking-widest uppercase">상담 결과 기록</p>
                    ...
                    <div className="mt-4 pt-4 border-t border-emerald-100 flex justify-between items-center text-[10px] font-black text-slate-400">
                        ...
                        <span>{req.completedAt && new Date(req.completedAt).toLocaleDateString()} 완료</span>
                    </div>
                  </div>
                )}
              </div>
